...
function (global, factory) {
  ...
  var TYPESET = (function () {
    ...
    var rHan = UNICODE.hanzi.base + '|' + UNICODE.hanzi.desc + '|' +
      UNICODE.hanzi.radical + '|' + rKana
    ...
    return {
      ...
      char: {
        ...
        kana: new RegExp('(' + rKana + '|' + rKanaS + '|' + rKanaH + ')',
          'g'),
      ...
      group: {
        ...
        punct: null,
        hanzi: new RegExp('(' + rHan + ')+', 'g'),
        western: new RegExp('(' + rLatn + '|' + rGk + '|' + rCy + '|' +
          rPt + ')+', 'ig'),
        kana: new RegExp('(' + rKana + '|' + rKanaS + '|' + rKanaH + ')+',
          'g'),
        eonmun: new RegExp('(' + rEon + '|' + rEonH + '|' + rPt + ')+',
          'g')
      },
      ...
      jinze: {
        ...
        touwei: new RegExp('(' + rBDOpen + '+)(' + rChar + ')(' + rBDEnd +
          '+)', 'ig'),
        tou: new RegExp('(' + rBDOpen + '+)(' + rChar + ')', 'ig'),
        wei: new RegExp('(' + rChar + ')(' + rBDEnd + '+)', 'ig'),
        middle: new RegExp('(' + rChar + ')(' + rBDMid + ')(' + rChar +
          ')', 'ig')
      },
      zhuyin: {
        form: new RegExp('^\u02D9?(' + rZyS + ')?(' + rZyJ + ')?(' +
          rZyY + ')?(' + rZyD + ')?$'),
      ...
      hws: {
        ...
        strict: [
        new RegExp('(' + rHan + ')' + rWhite + '?(' + rAlph + '|' + rPtOpen + ')',
            'ig'),
        new RegExp('(' + rAlph + '|' + rPtEnd + ')' + rWhite + '?(' + rHan + ')',
            'ig')
      ]
      },
  ...
  var Fibre =
    ...
    function (Finder) {
      ...
      function matches(node, selector, bypassNodeType39) {
        ...
        var matches = Efn.matches || Efn.mozMatchesSelector || Efn
          .msMatchesSelector || Efn.webkitMatchesSelector
      ...
      if (typeof document === 'undefined') throw new Error(
        'Fibre requires a DOM-supported environment.')
      ...
      Fibre.fn = Fibre.prototype = {
        ...
        init: function (context, noPreset) {
          ...
          if (!context) {
            ...
            if (context instanceof Document) this.context = context
              .body || context
        ...
        filterFn: function (node) {
          ...
          var result = matches(node, filter, true) && !matches(node,
            avoid)
          return (this.preset === 'prose') ? AVOID_NON_PROSE(node) &&
            result : result
        },
        boundaryFn: function (node) {
          var boundary = this.selector.boundary.join(', ') || null
          var result = matches(node, boundary)
          return (this.preset === 'prose') ? NON_INLINE_PROSE(node) ||
            result : result
        },
      ...
      (function () {
        ...
        function exposed() {
          ...
          return deprecated.apply(null, arguments) || findAndReplaceDOMText
            .apply(null, arguments)
        }

        function deprecated(regex, node, replacement, captureGroup,
          elFilter) {
          ...
          var instance = findAndReplaceDOMText(node, {
            ...
            replace: isReplacementFunction ? replacement : '$' + (
              captureGroup || '&'),
        ...
        exposed.NON_INLINE_PROSE = function (el) {
          return hasOwn.call(exposed.NON_CONTIGUOUS_PROSE_ELEMENTS, el
        ...
        exposed.PRESETS = {
          prose: {
            ...
            filterElements: function (el) {
              return !hasOwn.call(exposed.NON_PROSE_ELEMENTS, el
                .nodeName.toLowerCase())
            }
          }
        }
        ...
        Finder.prototype = {
          ...
          search: function () {
            ...
            var matches = []
            var self = this
            regex = typeof regex === 'string' ? RegExp(escapeRegExp(
              regex), 'g') : regex
              ...
              for (var i = 0, l = textAggregation.length; i < l; ++i) {
                ...
                if (regex.global) {
                  while (match = regex.exec(text)) {
                    matches.push(self.prepMatch(match, matchIndex++,
                      offset))
                  }
                } else {
          ...
          prepMatch: function (match, matchIndex, characterOffset) {
            ...
            match.endIndex = characterOffset + match.index + match[0]
              .length
          ...
          processMatches: function () {
            ...
            out: while (true) {
              if (curNode.nodeType === 3) {
                if (!endPortion && curNode.length + atIndex >= match
                  .endIndex) {
                  // We've found the ending
                  endPortion = {
                    node: curNode,
                    index: portionIndex++,
                    text: curNode.data.substring(match.startIndex -
                      atIndex, match.endIndex - atIndex),
                    indexInMatch: atIndex - match.startIndex,
                    indexInNode: match.startIndex -
                    atIndex, // always zero for end-portions
                ...
                if (!startPortion && curNode.length + atIndex > match
                  .startIndex) {
              ...
              doAvoidNode = curNode.nodeType === 1 && elementFilter &&
                !elementFilter(curNode)
              if (startPortion && endPortion) {
                curNode = this.replaceMatch(match, startPortion,
                  innerPortions, endPortion)
                // processMatches has to return the node that replaced the endNode
                // and then we step back so we can continue from the end of the 
                // match:
                atIndex -= (endPortion.node.data.length - endPortion
                  .endIndexInNode)
                startPortion = null
                endPortion = null
                innerPortions = []
          ...
          prepareReplacementString: function (string, portion, match,
            ...
            string = string.replace(/\$(\d+|&|`|')/g, function ($0, t) {
              ...
              case '&':
                ...
                break
              case '`':
                replacement = match.input.substring(0, match
                  .startIndex)
                break
              case '\'':
            ...
            return string.substring(portion.indexInMatch, portion
              .indexInMatch + portion.text.length)
          },
          ...
          matchIndex) {
            ...
            if (wrapper && wrapper.nodeType) {
              ...
              clone.innerHTML = wrapper.outerHTML || new XMLSerializer()
            ...
            var el = typeof wrapper == 'string' ? doc.createElement(
              wrapper) : wrapper
          ...
          replaceMatch: function (match, startPortion, innerPortions,
            endPortion) {
            ...
            if (matchStartNode === matchEndNode) {
              ...
              if (startPortion.indexInNode > 0) {
                ...
                preceedingTextNode = doc.createTextNode(node.data
                  .substring(0, startPortion.indexInNode))
              ...
              if (endPortion.endIndexInNode < node.length) { // ?????
                ...
                followingTextNode = doc.createTextNode(node.data
                  .substring(endPortion.endIndexInNode))
                node.parentNode.insertBefore(followingTextNode, node)
              }
              node.parentNode.removeChild(node)
              this.reverts.push(function () {
                if (preceedingTextNode === newNode
                  .previousSibling) {
              ...
              preceedingTextNode = doc.createTextNode(
                matchStartNode.data.substring(0, startPortion
                  .indexInNode)
              )
              ...
              for (var i = 0, l = innerPortions.length; i < l; ++i) {
                ...
                portion.node.parentNode.replaceChild(innerNode, portion
                  .node)
                this.reverts.push((function (portion, innerNode) {
                  return function () {
                    innerNode.parentNode.replaceChild(portion
                      .node, innerNode)
                  }
                }(portion, innerNode)))
              ...
              matchStartNode.parentNode.insertBefore(preceedingTextNode,
                matchStartNode)
              matchStartNode.parentNode.insertBefore(firstNode,
                matchStartNode)
              matchStartNode.parentNode.removeChild(matchStartNode)
              matchEndNode.parentNode.insertBefore(lastNode,
                matchEndNode)
              matchEndNode.parentNode.insertBefore(followingTextNode,
                matchEndNode)
              matchEndNode.parentNode.removeChild(matchEndNode)
              this.reverts.push(function () {
                preceedingTextNode.parentNode.removeChild(
                  preceedingTextNode)
                firstNode.parentNode.replaceChild(matchStartNode,
                  firstNode)
                followingTextNode.parentNode.removeChild(
                  followingTextNode)
                lastNode.parentNode.replaceChild(matchEndNode,
                  lastNode)
              })
              return lastNode
            }
  ...
  $.extend(Fibre.fn, {
      ...
      return (
        ...
        .replace(
          ...
          function (portion, match) {
            ...
            return ((portion.index === 0 && portion.isEnd) || portion
              .index === 1) ? elem : ''
          }
        )
        .replace(
    ...
    groupify: function (option) {
      ...
      if (option.hanzi || option.cjk) {
        this.wrap(
          TYPESET.group.hanzi, $.clone($.create('h-char-group',
            'hanzi cjk'))
        )
      }
      if (option.western) {
        this.wrap(
          TYPESET.group.western, $.clone($.create('h-word',
            'western'))
        )
      }
      if (option.kana) {
        this.wrap(
          TYPESET.group.kana, $.clone($.create('h-char-group',
            'kana'))
        )
      }
      if (option.eonmun || option.hangul) {
        this.wrap(
          TYPESET.group.eonmun, $.clone($.create('h-word',
            'eonmun hangul'))
        )
      }
      this.endAvoid()
      ...
      for (var j = 1; j <= 20; j++) {
        for (var i = 1; i <= 50; i++) {
          if (
            ...
            a.getImageData(i, j, 1, 1).data[3] !== b.getImageData(i, j, 1, 1)
            .data[3]
          ) {
            ret = false
            break
  ...
  Locale.support = (function () {
    ...
    function testCSSProp(prop) {
      ...
      var allProp = (prop + ' ' + PREFIX.join(ucProp + ' ') + ucProp)
        .split(' ')
      var ret
    ...
    return {
      ...
      'ruby-display': (function () {
        ...
        return div.querySelector('h-test-a').style.display ===
          'ruby' && div.querySelector('h-test-b').style.display ===
          'ruby-text-container'
      })(),
      'ruby-interchar': (function () {
        ...
        var div = $.create('div')
        var css
        div.innerHTML = '<h-test style="-moz-ruby-position:' + IC +
          ';-ms-ruby-position:' + IC + ';-webkit-ruby-position:' +
          IC + ';ruby-position:' + IC + ';"></h-test>'
        css = div.querySelector('h-test').style
        return css.rubyPosition === IC || css.WebkitRubyPosition ===
          IC || css.MozRubyPosition === IC || css.msRubyPosition ===
          IC
      })(),
  ...
  function renderSimpleRuby($ruby) {
    ...
    $
      ...
      .forEach(function ($rt) {
        ...
        $ru = clazz.contains('zhuyin') ? createZhuyinRu($rb, $rt) :
      ...
      .forEach(function ($rtc, order) {
        ...
        ret = $
          ...
          .map(function ($rt, i) {
            ...
            if (rbspan < span) {
              if (aRb.length > 1) {
                console.error('An impossible `rbspan` value detected.',
                  ruby)
                return
              }
              aRb = $.tag('rb', aRb[0])
  ...
  function getZhuyinHTML(rt) {
    ...
    diao = zhuyin
      ...
      .replace(/[\u02C5]/g, '\u02C7')
      .replace(/[\u030D]/g, '\u0358')
    return len === 0 ? '' : '<h-zhuyin length="' + len + '" diao="' + diao +
      '"><h-yin>' + yin + '</h-yin><h-diao>' + diao + '</h-diao></h-zhuyin>'
  }
  /**
    ...
    renderRuby: function (context, target) {
      ...
      $target
        .forEach(function ($ruby) {
          var clazz = $ruby.classList
          var $new
          if (clazz.contains('complex')) $new = renderComplexRuby(
            $ruby)
          else if (clazz.contains('zhuyin')) $new = SUPPORT_IC ?
            renderInterCharRuby($ruby) : renderSimpleRuby($ruby)
          // Finally, replace it
  ...
  function isVowelCombLigaNormal() {
    return createCompareFactory('"Romanization Sans"', '\u0061\u030D',
      '\uDB80\uDC61')
  }

  function isVowelICombLigaNormal() {
    return createCompareFactory('"Romanization Sans"', '\u0069\u030D',
      '\uDB80\uDC69')
  }

  function isZhuyinCombLigaNormal() {
    return createCompareFactory('"Zhuyin Kaiti"', '\u31B4\u0358',
      '\uDB8C\uDDB4')
  }
  ...
  $.extend(Han, {
    ...
    substVowelCombLiga: createSubstFactory(Han.TYPESET['display-as'][
      'comb-liga-vowel']),
    substZhuyinCombLiga: createSubstFactory(Han.TYPESET['display-as'][
      'comb-liga-zhuyin']),
    substCombLigaWithPUA: createSubstFactory(Han.TYPESET['display-as'][
      'comb-liga-pua']),
  ...
  $.extend(Han.fn, {
    ...
    substCombLigaWithPUA: function () {
      if (!Han.isVowelCombLigaNormal()) {
        ...
        this['comb-liga-vowel-i'] = Han.substVowelICombLiga(this
          .context)
      }
  ...
  window.addEventListener('DOMContentLoaded', function () {
    ...
    } else if (initContext = document.querySelector(
      '.han-init-context')) {
